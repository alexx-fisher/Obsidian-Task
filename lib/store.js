...
// Отсутствие колонки `note` в БД (миграция ещё не применена) не должно ломать
// создание/редактирование задач — в этом случае повторяем запрос без `note`.
const isMissingNote = (error) =>
  error && (error.code === 'PGRST204' || /note/i.test(error.message || '')) && /note/i.test(JSON.stringify(error));
...
export async function updateTask(task) {
  ...
  let { error } = await supabase.from('tasks').update(row).eq('id', task.id);
  if (error && isMissingNote(error)) {
    delete row.note;
    ({ error } = await supabase.from('tasks').update(row).eq('id', task.id));
  }
  if (error) console.error(error);
}
